...
//	bool bCVarOk = ConnectStudioRenderCVars( factory );
	...
	Q_StripLastDir(szGameConfigDir, sizeof(szGameConfigDir));	// Get rid of the 'bin' directory
	Q_StripLastDir(szGameConfigDir, sizeof(szGameConfigDir));	// Get rid of the 'bin' directory
	Q_StripLastDir(szGameConfigDir, sizeof(szGameConfigDir));	// Get rid of the 'bin' directory
	Q_AppendSlash(szGameConfigDir, sizeof(szGameConfigDir));
...
{
	CEditGameConfigs dlg(FALSE, GetMainWnd());
	if (dlg.DoModal() != IDOK)
	{
		return NULL;
	}
...
InitReturnVal_t CHammer::HammerInternalInit()
{
	...
	*/
	...
	if ( !initInfo.m_pDirectoryName[0] )
	{
		...
		Q_StripLastDir(pTempBuf, sizeof(pTempBuf));	// Get rid of the 'bin' directory
		Q_StripLastDir(pTempBuf, sizeof(pTempBuf));	// Get rid of the 'bin' directory
		Q_StripLastDir(pTempBuf, sizeof(pTempBuf));	// Get rid of the 'bin' directory
		Q_AppendSlash(pTempBuf, sizeof(pTempBuf));
		strcat( pTempBuf, "hl2" );
