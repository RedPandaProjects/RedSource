...
void DAS_SetTraceHeight( das_room_t *proom, trace_t *ptrU, trace_t *ptrD )
{
	...
	int zup = fabs(ptrU->endpos.z - ptrU->startpos.z);		// height above player's head
	int zdown = fabs(ptrD->endpos.z - ptrD->startpos.z);		// distance to floor from player's head
	int h;
	h = zup + zdown;
...
bool DAS_StartTraceChecks( das_room_t *proom )
{
	...
	if (trD.DidHit() && fabs(trD.endpos.z - trD.startpos.z) > 72)
		return false;
