	...
	KeyValues *&pSearchPaths )
{
	...
	if (!pMainFile)
	{
		Q_strncpy(gameinfoFilename, "..\\..\\..\\", sizeof(gameinfoFilename));
...
bool DoesFileExistIn2(const char* pDirectoryName, const char* pFilename)
{
	char filename[MAX_PATH];
	Q_strncpy(filename,"..\\..\\..\\" , sizeof(filename));
	Q_strncat(filename, pDirectoryName, sizeof(filename));
	Q_AppendSlash(filename, sizeof(filename));
	Q_strncat(filename, pFilename, sizeof(filename), COPY_ALL_CHARACTERS);
	Q_FixSlashes(filename);
	bool bExist = (_access(filename, 0) == 0);
...
FSReturnCode_t LocateGameInfoFile( const CFSSteamSetupInfo &fsInfo, char *pOutDir, int outDirLen )
{
	...
	if ( fsInfo.m_bOnlyUseDirectoryName )
	{
		...
		if ( !bExists )
		{
			bool bExists = DoesFileExistIn2(fsInfo.m_pDirectoryName, GAMEINFO_FILENAME);
			if (!bExists)
			{
